...
function simul(p; θ_0=0.0, ytn_0=0.0, ρ=0.0, T=100)
    (;ξ, γ, σ, κ) = p
    y_0 = θ_0
    π_0 = p.κ*(y_0 - ytn_0)
    sim_y = [y_0]
...
sl = Makie.SliderGrid(f[3,1],
    (label=L"\theta_0",  range = -0.5:0.01:0.5, startvalue=0.0),
    (label=L"y^{nt}_0", range = -0.5:0.01:0.5, startvalue=0.0),
)

sl_t, sl_y = sl.sliders
# sl_t = Makie.Slider(f[3,1],  range = -0.5:0.01:0.5, startvalue=0.0)
# sl_y = Makie.Slider(f[3,2], range = -0.5:0.01:0.5, startvalue=0.0)



function update()
    sy, sp = simul(p; θ_0=θ[], ytn_0=ytn[], ρ=ρ[])
