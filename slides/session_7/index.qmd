...
- Demande Agrégée: $$y_t = \theta_t - \sigma \gamma (\pi_t - \overline{\pi})$$
  - __mécanisme__: *les presssions inflationnistes ($\pi_t>\overline{\pi}$) poussent la banque centrale à adopter une politique monétaire restrictive, ce qui réduit la demande et la production[^footnote]*
- Offre Agrégée: $$\pi_t = \overline{ \pi } + \kappa (y_t - y^t_n)$$
...
[^footnote]: attention au rôle crucial de la banque centrale dans le modèle IS-MP! Sans la banque centrale, le modèle ne dit pas comment changent les taux réels lorsque les prix augmentent.
...
$$\text{\textbf{OA}}: \pi_t = \overline{\pi} + \kappa (y_t - y^t_n)$$

Dans ce modèle il y a:

- deux variables __endogènes__[^endo_exo]: $y_t$ et $\pi_t$
- deux variables __exogènes__: $\theta_t$ et $y^{n}_t$

[^endo_exo]: Par définition, les variables endogènes sont celles qui sont expliquées par le modèles, les variables exogènes sont déterminées par un mécanisme extérieur et sont prises comme données.

## Équilibre de court terme

On trouve aisément la paire (inflation $\pi_t$ / output $y_t$) qui correspond aux chocs ($\theta_t$, $y^n_t$ ).
...
![Choc persistant](assets/demand_shock_adjustment.png)
...
- Les chocs d'offre ne peuvent pas être directement annulés mais...
  ...
  - Les politiques de demande peuvent les *accomoder* à court terme (jusqu'à ce que les prix s'ajustent)